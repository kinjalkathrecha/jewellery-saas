...
class LabelTemplate(models.Model):
    shop = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text="Null means global preset template.",
    )
...
class BarcodeEvent(models.Model):
    ...
    sku_snapshot = models.CharField(
        max_length=100, blank=True, null=True, help_text="SKU identifier at the moment of log execution"
    )
    product = models.ForeignKey(
        "inventory.JewelleryItem", on_delete=models.SET_NULL, null=True, blank=True, related_name="barcode_events"
    )
    user = models.ForeignKey(
        "accounts.CustomUser", on_delete=models.SET_NULL, null=True, blank=True, related_name="barcode_events"
    )
