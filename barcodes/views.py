...
def print_tags_view(request):
    ...
    templates = LabelTemplate.objects.filter(
        Q(shop=request.shop) | Q(shop__isnull=True),
        is_active=True
    ...
    context = {
        ...
        'default_template': next((t for t in grouped_templates.values() if t.is_default), None) or next(iter(grouped_templates.values()), None)
    }
...
def item_lookup_api(request):
    ...
    if not code:
        return JsonResponse({
            ...
            'message': 'No lookup code provided.'
        }, status=400)
