...
class MetalRatesView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for metal_code, _ in MetalRate.METAL_CHOICES:
            rates = MetalRate.objects.filter(shop=shop, metal_type=metal_code).order_by('-effective_from', '-created_at')[:2]
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if updated_count > 0:
                messages.success(request, f"Successfully updated {updated_count} metal rate(s) and recalculated dependent jewellery prices!")
            else:
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not PlanPermissionService.check(request.shop, 'create_product'):
            if hasattr(request, 'subscription_locked') and request.subscription_locked:
                ...
                messages.error(request, "Product creation limit reached under your current subscription plan. Please upgrade your plan.")
