...
class MetalRate(models.Model):
    ...
    def get_current_rate(cls, shop, metal_type):
        rate_obj = cls.objects.filter(shop=shop, metal_type=metal_type).order_by('-effective_from', '-created_at').first()
...
class JewelleryItem(models.Model):
    ...
    barcode_type = models.CharField(max_length=20, choices=[('CODE128', 'Code 128'), ('EAN13', 'EAN 13'), ('QR', 'QR Code')], default='CODE128')
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['shop', 'design_code'], name='unique_shop_design_code')
        ]
        indexes = [
