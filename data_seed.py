...
def setup_qa():
    ...
    if not CustomUser.objects.filter(username="admin2").exists():
        CustomUser.objects.create_user(
            username="admin2", email="admin2@moonlight.com", password="adminpassword2", shop=shop_b, role="ADMIN"
        )
        print("Admin user 'admin2' created (password: adminpassword2)")

    if not CustomUser.objects.filter(username="staff2").exists():
        CustomUser.objects.create_user(
            username="staff2", email="staff2@moonlight.com", password="staffpassword2", shop=shop_b, role="STAFF"
        )
