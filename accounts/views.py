    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not PlanPermissionService.check(request.shop, 'add_staff'):
            if hasattr(request, 'subscription_locked') and request.subscription_locked:
                ...
                messages.error(request, "Staff member limit reached under your current subscription plan. Please upgrade your plan.")
        ...
        try:
            with transaction.atomic():
                ...
                Category.objects.create(
                    shop=shop,
                    name="Gold Rings",
                    description="Standard 22K/18K Gold Rings"
                )
                Category.objects.create(
                    shop=shop,
                    name="Diamond Necklaces",
                    description="Exquisite Diamond and Precious Stone Necklaces"
                )
            ...
            messages.success(self.request, "Your shop registration was successful! Please log in to access your dashboard.")
            return super().form_valid(form)
