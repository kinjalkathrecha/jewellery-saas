...
def home(request):
    ...
    repairs_delivered_today = Repair.objects.filter(shop=shop, status='DELIVERED', delivered_at__date=timezone.now().date(), is_active=True).count()
    repairs_overdue = Repair.objects.filter(shop=shop, is_active=True).exclude(status__in=['DELIVERED', 'CANCELLED']).filter(expected_delivery_date__lt=timezone.now().date()).count()
    
    # Repairs Revenue
    revenue_today = Repair.objects.filter(shop=shop, status='DELIVERED', delivered_at__date=timezone.now().date(), actual_cost__gt=0, is_active=True).aggregate(Sum('actual_cost'))['actual_cost__sum'] or 0
    revenue_month = Repair.objects.filter(shop=shop, status='DELIVERED', delivered_at__year=timezone.now().year, delivered_at__month=timezone.now().month, actual_cost__gt=0, is_active=True).aggregate(Sum('actual_cost'))['actual_cost__sum'] or 0
...
def business_reports_view(request):
    ...
    repairs_revenue = Repair.objects.filter(shop=shop, status='DELIVERED', is_active=True).aggregate(Sum('actual_cost'))['actual_cost__sum'] or 0.00
    
    # 2. Metal Distribution (Doughnut Chart Data)
    metal_data = InvoiceItem.objects.filter(
        invoice__shop=shop,
        item__isnull=False
    ).values('item__metal_type').annotate(
        qty=Sum('quantity'),
        sales=Sum('amount')
    )
    ...
    ).annotate(month=TruncMonth('delivered_at')).values('month').annotate(
        val=Sum('actual_cost')
    ).order_by('month')
    ...
    top_selling = InvoiceItem.objects.filter(
        invoice__shop=shop,
        item__isnull=False
    ).values('item__item_name', 'item__design_code', 'item__price').annotate(
